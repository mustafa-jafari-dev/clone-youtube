...
export default function MyChannelPage() {
  ...
  const { data: channel, isLoading: loadingChannel, error: channelError } = useChannel(channelId)
  const { data: videos, isLoading: loadingVideos } = useChannelVideos(channelId)

  if (isAuthLoading) return <ChannelSkeleton />

  if (!isLoggedIn) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20 text-muted-foreground">
        <User className="size-16" />
        <h1 className="text-xl font-semibold text-foreground">Your channel</h1>
        <p className="text-sm">Sign in to view and manage your channel.</p>
        ...
        <div className="flex flex-1 flex-col gap-2 sm:pt-2">
          ...
          <div className="mt-2 flex gap-2">
            <Button variant="outline" size="sm" asChild>
              <Link href={`/channel/${channel.id}`}>View channel</Link>
            </Button>
          </div>
        </div>
      </div>
